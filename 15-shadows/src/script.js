...
const spotLight = new THREE.SpotLight(0xffffff, 0.5, 10, 0.3 * Math.PI);
...
sphereMesh.rotation.x = -Math.PI * 0.5;
sphereMesh.position.y = plane.position.y + 0.01;
...
const tick = () => {
  ...
  sphere.position.y = Math.abs(Math.sin(elapsedTime * 2)) * 1.5;

  sphereMesh.position.x = Math.cos(elapsedTime) * 1.5;
  sphereMesh.position.z = Math.sin(elapsedTime) * 1.5;

  sphereMesh.material.opacity = (1 - sphere.position.y) * 0.3;
